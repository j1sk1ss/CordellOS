...
void UDP_send_packet(uint8_t* dst_ip, uint16_t src_port, uint16_t dst_port, void* data, int len) {
    ...
    udp_packet_t* packet = (udp_packet_t*)ALC_malloc(length, KERNEL);
...
void UDP_handle_packet(udp_packet_t* packet) {
    ...
    if (current_udp_packet >= PACKET_BUFFER_SIZE) {
        ...
        if (udp_packets[current_udp_packet].data_size != -1) {
            ALC_free(udp_packets[current_udp_packet].data, KERNEL);
            udp_packets[current_udp_packet].data_size = -1;            
        }
    }

    udp_packets[current_udp_packet].data = (uint8_t*)ALC_malloc(data_len, KERNEL);
...
udp_packet_body* UDP_pop_packet() {
    if (current_udp_packet >= 0) {
        ...
        udp_packet_body* packet = (udp_packet_body*)ALC_malloc(sizeof(udp_packet_body), KERNEL);
        packet->data = (uint8_t*)ALC_malloc(udp_packets[current_udp_packet].data_size, KERNEL);
