...
static void __pmem_fill(uint32_t color, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
...
void VESA_newline() {
    int font_height = psf_get_height(&_binary_src_kernel_font_psf_start);
...
static inline void __vmem_putc(int x, int y, char c, uint32_t foreground, uint32_t background) {
    __mem_putc(x, y, c, foreground, background, GFX_data.virtual_second_buffer);
}

static inline void __pmem_putc(int x, int y, char c, uint32_t foreground, uint32_t background) {
...
static void __vmem_fill(uint32_t color, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
    for (uint16_t x = a; x < c; x++) {
        for (uint16_t y = b; y < d; y++) {
...
uint8_t VESA_get_cursor_y() {
    return _cursor_y / psf_get_height(&_binary_src_kernel_font_psf_start);
}
...
uint8_t VESA_get_max_y() {
    return GFX_data.y_resolution / psf_get_height(&_binary_src_kernel_font_psf_start);
}
...
char __pmem_getc(int x, int y) {
    ...
    for (int row = 0; row < psf_get_height(&_binary_src_kernel_font_psf_start) * 8; row += 8) {
        memcpy(char_data + row, abs_row, 32);
        abs_row += step;
    }

    char character = 0;
    for (int i = 0; i < psf_get_height(&_binary_src_kernel_font_psf_start) * 8; i++) 
