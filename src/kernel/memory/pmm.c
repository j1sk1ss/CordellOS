...
static int _find_first_free_blocks(size_t num_blocks) {
    ...
    for (uint32_t i = 0; i < PMM_map.max_blocks / 32;  i++) {
        if (PMM_map.memory_map[i] != 0xFFFFFFFF) {
            for (int j = 0; j < 32; j++) {
                if (!(PMM_map.memory_map[i] & (1 << j))) {
                    for (uint32_t count = 0, PMM_free_blocks = 0; count < num_blocks; count++) {
                        if ((j + count > 31) && (i + 1 < PMM_map.max_blocks / 32)) {
                            if (!(PMM_map.memory_map[i + 1] & (1 << ((j + count) - 32)))) PMM_free_blocks++;
                        }
                        else {
                            if (!(PMM_map.memory_map[i] & (1 << (j + count)))) PMM_free_blocks++;
                        }

                        if (PMM_free_blocks == num_blocks) return i * 32 + j;
                    }
                }
            }
        }
